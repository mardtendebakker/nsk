...
export class ExactWebhookController {
  ...
  @Post('/order-payment')
  async handleOrderPayment(@Body() body: OrderPaymentWebhookDto, @Query('apiKey') apiKey: string) {
    const webhookApiKey = this.configService.get<string>('EXACT_WEBHOOK_API_KEY');
