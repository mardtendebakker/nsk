...
import {
  ...
  AUTOCOMPLETE_PRODUCT_SUB_TYPES_PATH,
...
export default function Form({
  ...
  const calculateListPrice = (): number => {
    ...
    const keys = Object.keys(formRepresentation).filter((key) =>
      key.includes(
        buildProductTypeKey({ id: formRepresentation.type_id.value })
      )
    );
    ...
    return keys.reduce(
      (accumulator, currentValue) =>
        accumulator +
        (formRepresentation[currentValue].additionalData?.selectedOption
          ?.price || 0),
      0
    );
  };
  ...
  return (
    <>
      <BorderedBox sx={{ p: '1rem' }}>
        <Typography sx={{ mb: '2rem' }} variant="h4">
          {trans('basicInfo')}
        </Typography>
        <Grid
          container
        ...
        >
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                setValue({ field: 'sku', value: e.target.value })
              }
              disabled={disabled}
              InputProps={{
            ...
            {formRepresentation.type_id.value && (
              <DataSourcePicker
                ...
                onChange={(selected: { id: number }) =>
                  setValue({ field: 'sub_type_id', value: selected?.id })
                }
          ...
          >
            <DataSourcePicker
              ...
              onChange={(selected: {
                ...
                setValue({
                  ...
                  value: selected?.id,
                  additionalData: {
                    location_template: selected?.location_template || [],
                  },
                });
              }}
            ...
            <TextField
              ...
              onChange={(e) =>
                setValue({ field: 'location_label', value: e.target.value })
              }
            ...
            <ProductStatusDataSourcePicker
              ...
              onChange={(selected: { id: number }) =>
                setValue({ field: 'status_id', value: selected?.id })
              }
            ...
            >
              ...
              <Typography
                ...
                sx={{ mt: '.5rem' }}
              >
                {trans('listPriceExtVat')}: {price(listPrice)}
              </Typography>
            </Box>
            <Box
              sx={{
          ...
          <Grid item sx={{ display: 'flex', flex: 1 }}>
            <TextField
              ...
              onChange={(e) =>
                setValue({ field: 'description', value: e.target.value })
              }
              disabled={disabled}
            />
          </Grid>
