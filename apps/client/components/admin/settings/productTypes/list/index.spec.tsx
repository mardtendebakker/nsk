...
describe('List', () => {
  ...
  it('handles search change properly', async () => {
    ...
    fireEvent.change(getByPlaceholderText('productTypesList.search.placeholder'), { target: { value: 'searchText' } });
  ...
  it('doesn\'t show create modal if button clicked and ajax performing', async () => {
    ...
    fireEvent.click(getByText('newProductType'));
    expect(queryByText('createProductType')).toBeNull();
  });
  it('shows create modal if button clicked', async () => {
    jest.spyOn(mockAxios, 'performing', 'get').mockReturnValue(false);
    const { getByText } = render(<List />);
    fireEvent.click(getByText('newProductType'));
    expect(getByText('createProductType')).not.toBeNull();
  });
  it('shows edit modal if button clicked', async () => {
    ...
    expect(getByText('editProductType')).not.toBeNull();
  });
});
