...
export default function EditModal({ onClose, onSubmit, id }: {
  ...
  const { data: productSubType, call, performing } = useAxios<undefined | ProductSubType>('get', PRODUCT_SUB_TYPES_PATH.replace(':id', id));
  const { call: callPut, performing: performingPut } = useAxios('put', PRODUCT_SUB_TYPES_PATH.replace(':id', id), { showSuccessMessage: true });
  const { formRepresentation, setValue, validate } = useForm(useMemo(() => initFormState(productSubType), [productSubType]));
  ...
  return (
    <ConfirmationDialog
      open
      title={<>{trans('editProductSubType')}</>}
      onClose={onClose}
