...
export default function EditModal({ onClose, onSubmit, id }: {
  ...
  const { data: team, call, performing } = useAxios<undefined | Team>('get', TEAMS_PATH.replace(':id', id));
  const { call: callPut, performing: performingPut } = useAxios('put', TEAMS_PATH.replace(':id', id), { showSuccessMessage: true });
  const { formRepresentation, setValue, validate } = useForm(useMemo(() => initFormState(team), [team]));
